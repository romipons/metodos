function trapecio(u0,v0,p0,q0,h,tiempo)
...
% Inicializo vector
	...
	while (k<k_max)
		...
		%Y(k+1)
		u(:,k+1) = u(:,k) + h/2*(p_(:,k+1) + p(:,k));
		v(:,k+1) = v(:,k) + h/2*(q_(:,k+1) + q(:,k));
		p(:,k+1) = p(:,k) + h/2*(((dividendo * v_(:,k+1))/divisor)  + ((dividendo_*v(:,k))/divisor_));
		q(:,k+1) = q(:,k) + h/2*(((dividendo * u_(:,k+1))/divisor)  + ((dividendo_*u(:,k))/divisor_));
		z(:,k+1)=u(:,k+1)*v(:,k+1);
